...
class QuadraticEquationSolver
{
    ...
    public Pair<Complex, Complex> solve(double a, double b, double c)
    {
        ...
        if(strategy instanceof  OrdinaryDiscriminantStrategy){
            x = new Complex(-b,0).plus(Complex.sqrt(strategy.calculateDiscriminant(a, b, c)));
            x = x.divides(new Complex(2*a, 0));

            y = new Complex(-b,0).minus(Complex.sqrt(strategy.calculateDiscriminant(a, b, c)));
            y  = y .divides(new Complex(2*a, 0));
        }

        if(strategy instanceof RealDiscriminantStrategy){
            Double discriminant = strategy.calculateDiscriminant(a, b, c);
            if(discriminant.isNaN()){
                x  = new Complex(NaN, NaN);
                y  = new Complex(NaN, NaN);
            } else {
                x = new Complex(-b,0).plus(Complex.sqrt(strategy.calculateDiscriminant(a, b, c)));
                x = x.divides(new Complex(2*a, 0));
...
public class Exercise {
    public static void main(String[] args) {
        QuadraticEquationSolver qs = new QuadraticEquationSolver(
                new RealDiscriminantStrategy());

        Pair<Complex, Complex> result = qs.solve(-1, 2, 3);
        System.out.println(result.first);
        System.out.println(result.second);

        QuadraticEquationSolver qs2 = new QuadraticEquationSolver(
                new OrdinaryDiscriminantStrategy());

        Pair<Complex, Complex> result2 = qs.solve(1, -6, -36);
        System.out.println(result2.first);
